$(() => {
	...
	const cropRoomDesignation = (num) => {
		...
		$roomsDesignation.text(
			`${roomsEnding.substr(0, MAX_LENGTH_ROOMS_MOB)}.`
		);
	};
	...
	$sellForm.on("submit", () => {
		if (!$sellForm.find(".is-error").length > 0) {
			...
			if (screen.width <= TAB_MIN) {
				if (
					$roomsDesignation.first().text().length >
					MAX_LENGTH_ROOMS_MOB
				) {
	...
	const onBuyerClick = (evt) => {
		if ($(evt.currentTarget).hasClass("customer-data-table__buyer-row")) {
			const $buyerCell = $(evt.currentTarget).find(
				".customer-data-table__item_buyer"
			);

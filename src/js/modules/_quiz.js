...
(function() {
	...
	function get_total_price(){
		...
		var total = numberWithSpaces(parseInt(price / time));
	...
	$quiz.find('.js-goto').on('click', function (e) {
        ...
        if($prevAll.length){
            $prevAll.each(function() {
                if(!$(this).hasClass('js-remove-ignore'))
                    $(this).remove()
                else
		...
		let bar_text = $last_done.attr('data-help') ? $last_done.data('help') : `+${$last_done.data('progress')}% за заполнение поля "${$last_done.find('.input__top .input__label').text()}"`
		...
		$done.each((i, el) => {
			...
			sub_total += $(el).attr('data-progress') ? Number($(el).data('progress')) : 0;
